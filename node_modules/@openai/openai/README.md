...
async function main() {
  ...
  for await (const chunk of stream) {
    Deno.stdout.writeSync(chunk.choices[0]?.delta?.content || '');
  }
}
...
import { OpenAIRealtimeWS } from "@openai/openai/beta/realtime/ws";
...
rt.on('response.text.delta', (event) => Deno.stdout.writeSync(event.delta));
...
const run = openai.beta.threads.runs
  ...
  .on('textCreated', (text) => Deno.stdout.writeSync('\nassistant > '))
  .on('textDelta', (textDelta, snapshot) => Deno.stdout.writeSync(textDelta.value))
  .on('toolCallCreated', (toolCall) => Deno.stdout.writeSync(`\nassistant > ${toolCall.type}\n\n`))
  .on('toolCallDelta', (toolCallDelta, snapshot) => {
    if (toolCallDelta.type === 'code_interpreter') {
      if (toolCallDelta.code_interpreter.input) {
        Deno.stdout.writeSync(toolCallDelta.code_interpreter.input);
      }
...
```typescript
import { AzureOpenAI } from "@openai/openai";
...
import "@openai/openai/shims/web";
import OpenAI from "@openai/openai";
```
